...
import { CURRENCY_VALUE_VND } from "../../../constant";
import {
  Dialog,
  DialogActions,
  DialogContent,
...
const UserDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      axios
        .get("api/trip?page=1&IsForUser=1", {
          headers: {
  ...
  const handleChangePage = async (event, value) => {
    ...
    if (haveSearched) {
      await axios
        .get(
          `api/trip?startProvinceID=${startProvinceID}&endProvinceID=${endProvinceID}&tripDate=${tripDate}&page=${value}&IsForUser=1`,
          {
            headers: {
              accessToken: localStorage.getItem("accessToken"),
            },
      ...
      await axios
        .get(`api/trip?page=${value}&IsForUser=1`, {
          headers: {
            accessToken: localStorage.getItem("accessToken"),
          },
        })
  ...
  const handleSearchTrip = async (event) => {
    ...
    await axios
      .get(
        `api/trip?startProvinceID=${startProvinceID}&endProvinceID=${endProvinceID}&tripDate=${tripDate}&IsForUser=1`,
        {
          headers: {
            accessToken: localStorage.getItem("accessToken"),
          },
  ...
  const handlePayment = async () => {
    const body = {
      TripID: trip.ID,
      BusSeatIDs: busSeatsChosenID,
      TotalPrice: trip.Price * busSeatsChosenID.length,
    };
    setLoading(true);
    const totalPrice = parseFloat(
      ((trip.Price * busSeatsChosenID.length) / CURRENCY_VALUE_VND.USD).toFixed(
        2
      )
    );
    ...
    await axios
      ...
      .then((res) => {
        ...
        axios
          .get(`api/paypal?totalPrice=${totalPrice}`, {
            headers: {
              accessToken: localStorage.getItem("accessToken"),
            },
          })
  ...
  return (
    <div>
          ...
          <Grid item xs={5}>
            <Paper elevation={10} style={{ paddingTop: "3%" }}>
              ...
              <Paper elevation={12} style={{ paddingTop: "3%" }}>
                ...
                <div style={{ display: "flex" }}>
                  <img
                    style={{ width: "15%", height: "50px", marginLeft: "40%" }}
                    src="https://logowik.com/content/uploads/images/visa-payment-card1873.jpg"
                    alt=""
                  />
                  <img
                    style={{ width: "16%", height: "32px", marginTop: "2%" }}
                    src="https://e7.pngegg.com/pngimages/289/163/png-clipart-paypal-business-logo-computer-icons-paypal-blue-text.png"
                    alt=""
                  />
                  <LoadingButton
                    color="success"
      ...
      >
        ...
        <DialogActions>
          <Button color="error" onClick={() => setOpenPayment(false)}>
            Cancel
          </Button>
          <Button
